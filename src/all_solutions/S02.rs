...
impl Solvable for Solution {
    ...
    fn second_solution(&self) -> String {
        ...
        let details = games.into_iter().map(|game| {
        ...
        let result: u32 = details
            .map(|detail| {
                detail.red.iter().max().unwrap()
                    * detail.green.iter().max().unwrap()
                    * detail.blue.iter().max().unwrap()
            })
            .sum();
            ...
            let cubes: Vec<Cube> = line_iter
                ...
                .map(|str_cube| {
                    ...
                    let cube_type: String =
                        str_cube.chars().filter(|c| c.is_alphabetic()).collect();
